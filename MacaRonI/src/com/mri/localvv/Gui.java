		...
		if (source == imageRegion) {
			if (source == imageRegion) {
				if (control.renderMode <= Control.SLICE_AND_BORDERS) {
					float[] xyzV = vv.trScreen2Vol(e.getX(), e.getY(), control.scaledDist);
					float xV = xyzV[0];
					float yV = xyzV[1];
					float zV = xyzV[2];
//					System.out.println("x= "+xV);
					if (xV >= 0 && xV < vv.vol.widthV && yV >= 0 && yV < vv.vol.heightV && zV >= 0 && zV < vv.vol.depthV) {
						...
						control.positionFactorX = xV/(float)maxPositionX;		
						float x = (vv.vol.widthV-1)*control.positionFactorX;
						
						positionX = (int)x;
						vv.cube.setCornersYZ(x);
						picSlice.drawSlices();
						...
						control.positionFactorY = yV/(float)maxPositionY;		
						float y = (vv.vol.widthV-1)*control.positionFactorY;
						positionY = (int) y;	
						vv.cube.setCornersXZ(y);
						picSlice.drawSlices();
						sliceImageRegion.findLines(vv.cube, control.scaledDist, vv.vol.depthV); 
						sliceImageRegion.setImage(picSlice.image);
						sliceImageRegion.repaint();

						control.positionFactorZ = (zV+1)/(float)maxPositionZ;
						float z = (vv.vol.depthV-1)*control.positionFactorZ;
						positionZ = (int)z;
						vv.cube.setCornersXY(z);
						picSlice.drawSlices();
						sliceImageRegion.findLines(vv.cube, control.scaledDist, vv.vol.depthV); 
						sliceImageRegion.setImage(picSlice.image);
						sliceImageRegion.repaint();
